...
- [x] **ANLY-01**: 系统可以记录目标脚本当前实现的功能和主要执行流程。
- [x] **ANLY-02**: 系统可以记录目标脚本的输入、输出、关键中间结果和副作用。
- [x] **ANLY-03**: 系统可以记录目标脚本依赖的本地文件、外部模块、数据文件和工作目录假设。
- [x] **ANLY-04**: 系统可以识别原始代码中是否存在随机过程，以及是否已经固定随机种子。
- [x] **ANLY-05**: 系统可以识别目标脚本在本轮有效分支中调用到的其它本地模块，并把这些模块纳入同一轮分析和重构范围。

### 方案设计

- [x] **DSGN-01**: 系统可以根据复杂度、复用价值和边界清晰度判断是否需要在 `src/LoPS` 下创建独立模块。
- [x] **DSGN-02**: 系统可以在修改实现前提出重构方案，覆盖功能拆分、文件拆分、模块边界、接口设计、数据路径和验证方式。
- [x] **DSGN-03**: 系统必须等待用户确认重构方案后，才进入实现修改。
- [x] **DSGN-04**: 当某段逻辑不适合模块化时，系统可以记录原因并提出更轻量的整理方式。
...
- [x] **ARCH-01**: 重构不是简单搬运代码，必须基于原始行为重新设计模块结构、接口和实现。
- [x] **ARCH-02**: 新设计必须追求高内聚低耦合，清晰区分核心计算、数据读取、参数配置、运行入口和验证逻辑。
- [x] **ARCH-03**: 重构方案必须恪守 KISS 原则，优先选择最直接、最清晰、最易维护的实现。
- [x] **ARCH-04**: 系统必须避免过度工程化、过早抽象和不必要的防御性设计；任何新增抽象都必须有明确收益。
