"""generate_grammar 验证脚本测试。

覆盖精确值比较、差异路径报告和验证适配器输出字段。
"""
...
from script.generate_grammar.legacy_adapter import LEGACY_FIELD_ORDER, convert_generate_grammar_output_to_legacy
from script.generate_grammar.validate_generate_grammar import compare_legacy_dict, compare_values
...
class GenerateGrammarValidationTest(unittest.TestCase):
    ...
    def test_compare_values_reports_path_for_different_values(self) -> None:
        """验证值不一致时差异报告包含精确路径。"""
        # 差异报告必须包含精确 key path，便于定位输出不一致的位置。
    ...
    def test_compare_legacy_dict_reports_missing_key(self) -> None:
        """验证基准字段缺失时会生成 missing key 差异。"""
        # 基准 pickle 中存在的 key 如果在待验输出缺失，必须明确报告。
    ...
    def test_legacy_adapter_converts_representative_output_for_baseline_comparison(self) -> None:
        """验证代表性文件输出可转换为基准字段集合。"""
        # 代表性真实文件检查：核心输出先保持结构化，再由脚本层适配接口转换为基准字段。
